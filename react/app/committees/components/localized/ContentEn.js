import React from 'react';

const ContentEn = () => (
  <div>
    <h3 id="editorial">
      Editorial Committee
    </h3>
    <div className="members-full">
      <div className="member-full">
        <h4 className="name">
          Richard Conte
        </h4>
        <p className="desc">
          Directeur de la publication. Artiste et Professeur des Universités en Arts et Sciences de l’Art à l’Université Paris 1 UMR ACTE 8218.
        </p>
      </div>
      <div className="member-full">
        <h4 className="name">
          Bernard Darras
        </h4>
        <p className="desc">
          Responsable éditorial et rédacteur en chef. Professeur des Universités en Sémiotique à l’Université Paris 1 UMR ACTE 8218.
        </p>
      </div>
      <div className="member-full">
        <h4 className="name">
          Christophe Génin
        </h4>
        <p className="desc">
          Professeur de philosophie de l’art et d’études culturelles à Paris 1 Panthéon-Sorbonne.
        </p>
      </div>
    </div>
    <h3 id="reading">
      Peer review
    </h3>
    <p className="committee-desc">
      Ce Comité de lecture internationnal et pluridisciplinaire est chargé d’évaluer en double aveugle la rigueur scientifique de chaque article et sa conformité au projet éditorial relationniste de WikiCreation.
    </p>
    <ul className="members">
      <li>
        Elisabeth Amblard
      </li>
      <li>
        Jan Baetens
      </li>
      <li>
        Dominique Berthet
      </li>
      <li>
        Marlène Biton
      </li>
      <li>
        Karen Brunel-Lafargue
      </li>
      <li>
        Cristina Castellano
      </li>
      <li>
        Yichien Cooper
      </li>
      <li>
        Agnès Foiret-Collet
      </li>
      <li>
        Barbara Formis
      </li>
      <li>
        Pierre-Henry Frangne
      </li>
      <li>
        Karen O’Rourke
      </li>
      <li>
        Hélène Sirven
      </li>
      <li>
        Apolline Torregrosa
      </li>
      <li>
        Stéphane Vial
      </li>
      <li>
        Patricia Villeneuve
      </li>
      <li>
        Fernando Zalamea
      </li>
    </ul>
    <h3 id="scientific">
      Scientific committee
    </h3>
    <div className="members-full small">
      <div className="member-full">
        <h4 className="name">
          Imanol Aguirre Arriaga
        </h4>
        <p className="desc">
          Universidad Publica de Navarra, España
        </p>
      </div>
      <div className="member-full">
        <h4 className="name">
          Jan Baetens
        </h4>
        <p className="desc">
          Université de Leuven, Belgique
        </p>
      </div>
      <div className="member-full">
        <h4 className="name">
          Ana Mae Barbosa
        </h4>
        <p className="desc">
          USP, Sao Paulo, Brasil
        </p>
      </div>
      <div className="member-full">
        <h4 className="name">
          Anne Beyaert
        </h4>
        <p className="desc">
          Université de Bordeaux 3, France
        </p>
      </div>
      <div className="member-full">
        <h4 className="name">
          Dominique Berthet
        </h4>
        <p className="desc">
          Université des Antilles et de Guyanne, France
        </p>
      </div>
      <div className="member-full">
        <h4 className="name">
          Bernhard Bürdek
        </h4>
        <p className="desc">
          HfG Offenbach, Deutschland
        </p>
      </div>
      <div className="member-full">
        <h4 className="name">
          Agnès Callu
        </h4>
        <p className="desc">
          Conservateur du patrimoine au Musée des arts décoratifs, France
        </p>
      </div>
      <div className="member-full">
        <h4 className="name">
          Dominique Chateau
        </h4>
        <p className="desc">
          Université Paris 1 Panthéon-Sorbonne, France
        </p>
      </div>
      <div className="member-full">
        <h4 className="name">
          Solange Coutinho
        </h4>
        <p className="desc">
          UFP Recife, Brasil
        </p>
      </div>
      <div className="member-full">
        <h4 className="name">
          Rejane Coutinho
        </h4>
        <p className="desc">
          UNESP Sao Paulo, Brasil
        </p>
      </div>
      <div className="member-full">
        <h4 className="name">
          Philippe Dagen
        </h4>
        <p className="desc">
          Université Paris 1 Panthéon-Sorbonne, France
        </p>
      </div>
      <div className="member-full">
        <h4 className="name">
          Filippo Fimiani
        </h4>
        <p className="desc">
          Università di Salerno, Italia
        </p>
      </div>
      <div className="member-full">
        <h4 className="name">
          Alain Findeli
        </h4>
        <p className="desc">
          Université de Nîmes, France
        </p>
      </div>
      <div className="member-full">
        <h4 className="name">
          Geneviève Fraisse
        </h4>
        <p className="desc">
          CNRS, France
        </p>
      </div>
      <div className="member-full">
        <h4 className="name">
          Pierre-Henry Frangne
        </h4>
        <p className="desc">
          Université de Rennes 2, France
        </p>
      </div>
      <div className="member-full">
        <h4 className="name">
          Nathalie Heinich
        </h4>
        <p className="desc">
          EHESS, France
        </p>
      </div>
      <div className="member-full">
        <h4 className="name">
          Norbert Hillaire
        </h4>
        <p className="desc">
          Université de Nice, France
        </p>
      </div>
      <div className="member-full">
        <h4 className="name">
          Marc Jimenez
        </h4>
        <p className="desc">
          Université Paris 1 Panthéon-Sorbonne, France
        </p>
      </div>
      <div className="member-full">
        <h4 className="name">
          Andrea Kárpáti
        </h4>
        <p className="desc">
          Eötvös Loránd University, Hungaria
        </p>
      </div>
      <div className="member-full">
        <h4 className="name">
          Derrick de Kerckhove
        </h4>
        <p className="desc">
          Federico II de Naple
        </p>
      </div>
      <div className="member-full">
        <h4 className="name">
          Olga Kisseleva
        </h4>
        <p className="desc">
          Université Paris 1 Panthéon-Sorbonne, France
        </p>
      </div>
      <div className="member-full">
        <h4 className="name">
          Anna Kindler
        </h4>
        <p className="desc">
          University of British Columbia, Canada
        </p>
      </div>
      <div className="member-full">
        <h4 className="name">
          Jacinto Lageira
        </h4>
        <p className="desc">
          Université Paris 1 Panthéon-Sorbonne, France
        </p>
      </div>
      <div className="member-full">
        <h4 className="name">
          Sandra Laugier
        </h4>
        <p className="desc">
          Université Paris 1 Panthéon-Sorbonne, France
        </p>
      </div>
      <div className="member-full">
        <h4 className="name">
          Martin Lefebvre
        </h4>
        <p className="desc">
          Université Concordia, Canada
        </p>
      </div>
      <div className="member-full">
        <h4 className="name">
          Thierry Lenain
        </h4>
        <p className="desc">
          Université Libre de Bruxelles, Belgique
        </p>
      </div>
      <div className="member-full">
        <h4 className="name">
          Rachel Mason
        </h4>
        <p className="desc">
          University of Roehampton, London, UK
        </p>
      </div>
      <div className="member-full">
        <h4 className="name">
          Marianne Massin
        </h4>
        <p className="desc">
          Université de Lille 3, France
        </p>
      </div>
      <div className="member-full">
        <h4 className="name">
          Stéphane Natkin
        </h4>
        <p className="desc">
          CNAM, France
        </p>
      </div>
      <div className="member-full">
        <h4 className="name">
          Françoise Parfait
        </h4>
        <p className="desc">
          Université Paris 1 Panthéon-Sorbonne, France
        </p>
      </div>
      <div className="member-full">
        <h4 className="name">
          David Pariser
        </h4>
        <p className="desc">
          Université Concordia, Canada
        </p>
      </div>
      <div className="member-full">
        <h4 className="name">
          Michael Parsons
        </h4>
        <p className="desc">
          University of Illinois, USA
        </p>
      </div>
      <div className="member-full">
        <h4 className="name">
          Bruno Péquignot
        </h4>
        <p className="desc">
          Université Sorbonne Nouvelle, France
        </p>
      </div>
      <div className="member-full">
        <h4 className="name">
          Jean-Marie Schaeffer
        </h4>
        <p className="desc">
          Université Paris 1 Panthéon-Sorbonne, France
        </p>
      </div>
      <div className="member-full">
        <h4 className="name">
          Rachida Triki
        </h4>
        <p className="desc">
          Université de Tunis, Tunisie
        </p>
      </div>
      <div className="member-full">
        <h4 className="name">
          Patricia Villeneuve
        </h4>
        <p className="desc">
          Florida State University, USA
        </p>
      </div>
      <div className="member-full">
        <h4 className="name">
          Stéphane Vial
        </h4>
        <p className="desc">
          Université de Nîmes, France
        </p>
      </div>
      <div className="member-full">
        <h4 className="name">
          Christophe Viart
        </h4>
        <p className="desc">
          Université de Rennes 2, France
        </p>
      </div>
    </div>
  </div>
);

module.exports = ContentEn;
